...
func (h *Handler) handlePipelineNodeTypes(w http.ResponseWriter, r *http.Request) {
	...
	out = append(out,
		...
		nodeTypeDescriptor{
			Type:        model.NodeTypeModelRef,
			Label:       "Référence de modèle",
			Description: "Émet le nom d'un modèle (classique ou virtuel) choisi dans le catalogue, à brancher sur tout port model_name.",
			InputPorts:  []*proto.PortDescriptor{},
			OutputPorts: []*proto.PortDescriptor{
				{Name: "model_name", PortType: string(model.PortTypeString)},
			},
			ConfigSchema: `{"type":"object","properties":{"proxyName":{"type":"string","title":"Modèle"}}}`,
		},
		nodeTypeDescriptor{
			Type:        model.NodeTypeModelFallback,
			Label:       "Modèle avec repli",
			Description: "Appelle le premier modèle de la liste et passe au suivant en cas d'échec (erreur, délai, 429). Modèles réels uniquement.",
			InputPorts: []*proto.PortDescriptor{
				{Name: "request", PortType: string(model.PortTypeRequest), Required: true},
				{Name: "model_name", PortType: string(model.PortTypeString)},
			},
			...
			ConfigSchema: `{"type":"object","properties":{"models":{"type":"array","title":"Modèles, par ordre de préférence","items":{"type":"string"}}}}`,
		},
		nodeTypeDescriptor{
		...
		nodeTypeDescriptor{
			...
			Label:       "Sélection",
			Description: "Émet l'une de deux chaînes selon un booléen.",
			InputPorts: []*proto.PortDescriptor{
				{Name: "condition", PortType: string(model.PortTypeBoolean), Required: true},
				{Name: "when_true", PortType: string(model.PortTypeString), Required: true},
				{Name: "when_false", PortType: string(model.PortTypeString), Required: true},
			},
			OutputPorts: []*proto.PortDescriptor{
				{Name: "value", PortType: string(model.PortTypeString)},
			},
		...
		nodeTypeDescriptor{
			...
			InputPorts: []*proto.PortDescriptor{
				{Name: "a", PortType: string(model.PortTypeNumber)},
				{Name: "b", PortType: string(model.PortTypeNumber)},
				{Name: "c", PortType: string(model.PortTypeNumber)},
				{Name: "d", PortType: string(model.PortTypeNumber)},
			},
			OutputPorts: []*proto.PortDescriptor{
				{Name: "result", PortType: string(model.PortTypeNumber)},
			},
			ConfigSchema: `{"type":"object","properties":{"op":{"type":"string","title":"Opération","enum":["sum","avg","min","max","product","weighted"],"default":"sum"},"weights":{"type":"array","title":"Poids de a, b, c, d (moyenne pondérée)","items":{"type":"number"}}}}`,
		},
		nodeTypeDescriptor{
			Type:        model.NodeTypeSample,
			Label:       "Échantillonnage",
			Description: "Sélectionne un pourcentage du trafic, de façon stable par utilisateur ou par jeton, pour un déploiement progressif.",
			InputPorts:  []*proto.PortDescriptor{},
			OutputPorts: []*proto.PortDescriptor{
				{Name: "selected", PortType: string(model.PortTypeBoolean)},
				{Name: "bucket", PortType: string(model.PortTypeNumber)},
			},
			ConfigSchema: `{"type":"object","properties":{"percent":{"type":"number","title":"Pourcentage sélectionné","minimum":0,"maximum":100,"default":10},"key":{"type":"string","title":"Stabilité","description":"user : même côté pour un même utilisateur ; token : par jeton ; random : tirage à chaque requête.","enum":["user","token","random"],"default":"user"},"salt":{"type":"string","title":"Sel","description":"Change la répartition sans changer le pourcentage."}}}`,
		},
		nodeTypeDescriptor{
			Type:        model.NodeTypeContext,
			Label:       "Contexte",
			Description: "Expose ce que la passerelle sait de la requête : utilisateur, organisation, jeton, modèle demandé, heure et jour.",
			InputPorts:  []*proto.PortDescriptor{},
			OutputPorts: []*proto.PortDescriptor{
				{Name: "user_id", PortType: string(model.PortTypeString)},
				{Name: "org_id", PortType: string(model.PortTypeString)},
				{Name: "token_id", PortType: string(model.PortTypeString)},
				{Name: "display_name", PortType: string(model.PortTypeString)},
				{Name: "requested_model", PortType: string(model.PortTypeString)},
				{Name: "hour", PortType: string(model.PortTypeNumber)},
				{Name: "weekday", PortType: string(model.PortTypeString)},
				{Name: "is_weekend", PortType: string(model.PortTypeBoolean)},
			},
		...
		nodeTypeDescriptor{
			Type:         model.NodeTypeNote,
			Label:        "Note",
			Description:  "Texte libre sur le canevas, sans effet à l'exécution.",
			InputPorts:   []*proto.PortDescriptor{},
			OutputPorts:  []*proto.PortDescriptor{},
			ConfigSchema: `{"type":"object","properties":{"text":{"type":"string","title":"Texte","format":"multiline"}}}`,
		},
		nodeTypeDescriptor{
