...
| `XOLO_PROXY_ACTIVE_USER_COUNT_TIMEOUT` | `10s` | Durée maximale d'un comptage d'utilisateurs actifs. Le comptage tourne indépendamment de la requête proxy qui l'a déclenché ; au-delà de cette durée il est abandonné, l'échec est mémorisé pour la durée du cache et la part de chaque utilisateur retombe sur l'effectif complet. Si le compteur `xolo_fair_share_degraded_shares_total` monte en continu, c'est ce délai qu'il faut relever, ou l'index de la table d'usage qu'il faut vérifier. |
