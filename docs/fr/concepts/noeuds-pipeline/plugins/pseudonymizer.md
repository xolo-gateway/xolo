...
Plusieurs formes traversent sans réécriture, chacune pour une raison différente — mais jamais en silence : ce qui part sans être réécrit est scanné en lecture seule (`Detect`, jamais `Anonymize` : pas de session, rien de réécrit) et vient alimenter l'événement `sensitive-data.detected` sous `leak_entities`/`leak_types`, tenus à part de `entities`/`types` qui ne comptent, eux, que ce qui a été effectivement pseudonymisé. `entities` et `leak_entities` dénombrent tous deux des valeurs distinctes, et `leak_types` les répartit sur la même base : une même adresse citée cinq fois dans un raisonnement compte pour une fuite, pas cinq, et la même valeur écrite de deux façons n'en fait qu'une. Deux réserves sur ce scan : les champs qui ne portent jamais de texte libre — `encrypted_content`, `data`, `signature`, les identifiants — ne sont pas lus quand ils portent une chaîne, car les relire à chaque tour d'une conversation agentique coûte cher pour ne rien trouver ; et une entrée de `content` qui n'est pas un objet JSON (une chaîne nue) est transmise sans être scannée.
