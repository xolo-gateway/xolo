...
type PostResponseInput struct {
	...
	// response_tool_calls_json carries the tool calls the model emitted, as a
	// JSON array of {"id","name","arguments"} objects, `arguments` being the
	// JSON document the client will execute. A node that rewrote the request
	// has to be able to undo its own rewriting here too: a placeholder left in
	// a tool call reaches the client verbatim and the call runs against it.
	ResponseToolCallsJson string `protobuf:"bytes,8,opt,name=response_tool_calls_json,json=responseToolCallsJson,proto3" json:"response_tool_calls_json,omitempty"`
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}
...
func (x *PostResponseInput) GetResponseToolCallsJson() string {
	if x != nil {
		return x.ResponseToolCallsJson
	}
	return ""
}
...
type PostResponseOutput struct {
	...
	// modified_tool_calls_json, when non-empty, replaces the response tool calls.
	// Same shape as response_tool_calls_json; only `arguments` is read back, the
	// id and the name of a call are never taken from the plugin. Return the array
	// in the order it was received: calls are matched back by position, because
	// an id is not always there to match on.
	ModifiedToolCallsJson string `protobuf:"bytes,2,opt,name=modified_tool_calls_json,json=modifiedToolCallsJson,proto3" json:"modified_tool_calls_json,omitempty"`
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}
...
func (x *PostResponseOutput) GetModifiedToolCallsJson() string {
	if x != nil {
		return x.ModifiedToolCallsJson
	}
	return ""
}
...
const file_pkg_pluginsdk_proto_plugin_proto_rawDesc = "" +
	...
	"\x13no_response_rewrite\x18\a \x01(\bR\x11noResponseRewrite\"\xcd\x02\n" +
	...
	"\n" +
	"node_state\x18\a \x01(\fR\tnodeState\x127\n" +
	"\x18response_tool_calls_json\x18\b \x01(\tR\x15responseToolCallsJson\"\x89\x01\n" +
	"\x12PostResponseOutput\x12:\n" +
	"\x19modified_response_content\x18\x01 \x01(\tR\x17modifiedResponseContent\x127\n" +
	"\x18modified_tool_calls_json\x18\x02 \x01(\tR\x15modifiedToolCallsJson\"\xf0\x02\n" +
